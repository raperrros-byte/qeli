...
public static class Loc
{
    ...
    private static readonly Dictionary<string, (string en, string ru)> Strings = new()
    {
        ...
        ["TrafficMode"] = ("Traffic (all profiles)", "Трафик (все профили)"),
        ["TrafficPresetFull"] = ("1. Full tunnel — all apps via VPN", "1. Полный туннель — весь трафик через VPN"),
        ["TrafficPresetApps"] = ("2. Selected apps only — rest direct", "2. Только выбранные приложения — остальное direct"),
        ["TrafficPresetProxy"] = ("3. Local SOCKS/HTTP proxy — apps opt-in", "3. Локальный SOCKS/HTTP прокси — приложения сами"),
        ["TrafficPresetIntercept"] = ("4. Force selected apps into local proxy (needs ProxyBridge)",
            "4. Перехват: выбранные приложения → в локальный прокси (нужен ProxyBridge)"),
        ["TrafficPresetAppsProxy"] = ("3. Local SOCKS/HTTP proxy — apps opt-in", "3. Локальный SOCKS/HTTP прокси — приложения сами"),
        ["TrafficModeTunnel"] = ("Full tunnel", "Полный туннель"),
        ["TrafficModeSplit"] = ("Split tunnel (no system gateway)", "Split-туннель (без системного gateway)"),
        ["TrafficModeProxy"] = ("Local SOCKS/HTTP proxy", "Локальный SOCKS/HTTP прокси"),
        ["TrafficModeProxyHint"] = (
            "Mode 3: set apps to 127.0.0.1:port. Mode 4: ProxyBridge forces TCP of selected apps into that SOCKS (install https://github.com/InterceptSuite/ProxyBridge).",
            "Режим 3: укажите в приложениях 127.0.0.1:порт. Режим 4: ProxyBridge принудительно гонит TCP выбранных приложений в этот SOCKS (нужен https://github.com/InterceptSuite/ProxyBridge)."),
        ["ProxyBridgeMissing"] = (
            "ProxyBridge_CLI.exe not found — install ProxyBridge or set its path. Manual proxy (mode 3) still works.",
            "ProxyBridge_CLI.exe не найден — установите ProxyBridge или укажите путь. Ручной прокси (режим 3) работает."),
        ["TrafficModeAppsHint"] = (
            "Pick applications for modes 2 and 4. DNS below applies to every profile.",
            "Выберите приложения для режимов 2 и 4. DNS ниже применяется ко всем профилям."),
        ["TrafficModeApplied"] = ("Applied to all profiles", "Применено ко всем профилям"),
        ["TrafficGatewayWhenAll"] = ("System routing (when all apps)", "Системный маршрут (когда все приложения)"),
        ["NeedApps"] = ("Select applications for this mode first.",
            "Сначала выберите приложения для этого режима."),
        ["NeedAppsPick"] = ("Pick at least one application first.",
            "Сначала выберите хотя бы одно приложение."),
        ["GlobalRoutingManaged"] = (
            "Application filter, DNS and local proxy are set on the main window for all profiles.",
            "Фильтр приложений, DNS и локальный прокси задаются на главном окне для всех профилей."),
