...
public static class Loc
{
    ...
    private static readonly Dictionary<string, (string en, string ru)> Strings = new()
    {
        ...
        ["TrafficModeProxyHint"] = (
            "Proxy can run with per-app tunnelling: selected apps use WinDivert; other apps can use 127.0.0.1:port.",
            "Прокси работает вместе с туннелем приложений: выбранные приложения — через WinDivert, остальные могут использовать 127.0.0.1:порт."),
        ["TrafficModeAppsHint"] = (
            "Application filter and DNS below apply to every profile immediately.",
            "Фильтр приложений и DNS ниже применяются сразу ко всем профилям."),
        ...
        ["NeedApps"] = ("Select applications with “Select applications…”, then the filter will apply.",
            "Выберите приложения кнопкой «Выбрать приложения…», затем фильтр применится."),
