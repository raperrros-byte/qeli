...
public static class Loc
{
    ...
    private static readonly Dictionary<string, (string en, string ru)> Strings = new()
    {
        ...
        ["TrafficModeTunnel"] = ("Full tunnel", "Туннельный режим"),
        ...
        ["SniSpeedHint"] = (
            "Runs every selected transport profile against every selected SNI (TLS handshake + short HTTPS download). Use Select all / delay between probes to avoid bursts.",
            "Прогоняет каждый выбранный профиль/режим против каждого выбранного SNI (TLS handshake + короткая HTTPS-загрузка). «Выбрать все» и пауза между пробами — чтобы не долбить хосты пачкой."),
