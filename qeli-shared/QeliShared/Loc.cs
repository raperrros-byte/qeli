...
public static class Loc
{
    ...
    private static readonly Dictionary<string, (string en, string ru)> Strings = new()
    {
        ...
        ["TrafficPresetAppsProxy"] = ("3. Selected apps + local proxy — rest direct", "3. Выбранные приложения + локальный прокси — остальное direct"),
        ...
        ["TrafficModeProxyHint"] = (
            "In mode 3, selected apps use WinDivert; other apps can use 127.0.0.1:port.",
            "В режиме 3 выбранные приложения идут через WinDivert; остальные могут использовать 127.0.0.1:порт."),
        ["TrafficModeAppsHint"] = (
            "Pick applications for modes 2 and 3. DNS below applies to every profile.",
            "Выберите приложения для режимов 2 и 3. DNS ниже применяется ко всем профилям."),
