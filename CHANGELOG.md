...
## Fork [raperrros-byte/qeli](https://github.com/raperrros-byte/qeli) — 2026-09-15

Maintained by: Daniil Nekrasov \<raperrros@yandex.ru\>

Based on upstream [`v0.8.1`](https://github.com/litvinovtd/qeli/releases/tag/v0.8.1).
Fork-only features kept on top of upstream: auto-transport / Mode × SNI matrix, geo routing,
local SOCKS/HTTP proxy, nginx SNI + `public_port`, multi-profile CLI/panel INI export,
Android geo routing off main thread, Windows global traffic / bulk delete / panel metrics,
reality session_id seal, signed Android release pipeline.

Full upstream changelog follows in the version sections above and below.
