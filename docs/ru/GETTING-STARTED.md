...
**Форк — авто-транспорт и SNI.** На Windows рядом с **Подключить** есть чекбокс **Авто**
(матрица режимы × выбранные SNI) и тумблер авто-транспорта на главном экране; окно ⚡ —
тест SNI/режимов. На Android: вкладка **SNI**, авто-транспорт и Apply SNI на главном.
CLI: `auto_transport = true`, `qeli probe-transports`, `qeli speedtest`. Панель:
`/api/transport/modes`, `/api/transport/sni-presets`, `/api/speedtest` — см.
[PANEL.md](PANEL.md) и [CHANGELOG.md](../../CHANGELOG.md).
